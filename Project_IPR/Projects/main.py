...
class HandMagicApp(QMainWindow):
    def __init__(self):
        ...
        # Initialize image adjustment settings
    ...
    def start_camera(self):
        ...
        try:
            if not self.cap or not self.cap.isOpened():
                ...
                # Try to apply hardware settings
    ...
    def update_frame(self):
        ...
        try:
            ...
            # Convert RGB back to BGR (to fix any color display issues)
            ...
            if results.multi_hand_landmarks:
                for hand_landmarks in results.multi_hand_landmarks:
                    ...
                    palm_x = int(sum(hand_landmarks.landmark[i].x for i in palm_landmarks) / 
                                len(palm_landmarks) * self.cam_width)
                    palm_y = int(sum(hand_landmarks.landmark[i].y for i in palm_landmarks) / 
                                len(palm_landmarks) * self.cam_height)
                    
                    # Get index finger position for rainbow
                    ...
                    if self.is_hand_open(hand_landmarks):
                        ...
                        if self.current_effect == "Fireworks":
                            frame_bgr = self.fireworks_effect.draw_firework(
                                frame_bgr, palm_x, palm_y, size=self.effect_intensity)
                        elif self.current_effect == "Sparkles":
                            frame_bgr = self.sparkles_effect.draw_sparkles(
                                frame_bgr, palm_x, palm_y, intensity=self.effect_intensity/100)
                        elif self.current_effect == "Fire":
                            frame_bgr = self.fire_effect.draw_fire(
                                frame_bgr, palm_x, palm_y, size=self.effect_intensity*2)
                    else:
    ...
    def update_camera_settings(self, hardware_only=False):
        """Update camera brightness/contrast based on sliders"""
        self.brightness_adjust = self.brightness_slider.value()
        self.contrast_adjust = self.contrast_slider.value() / 100
        
        # Try to set hardware controls first (works on some cameras)
